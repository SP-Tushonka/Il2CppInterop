    ...
    internal unsafe class Class_GetFieldDefaultValue_Hook : Hook<Class_GetFieldDefaultValue_Hook.MethodDelegate>
    {
        ...
        public override void TargetMethodNotFound()
        {
            Logger.Instance.LogWarning(
                "Class::GetDefaultFieldValue did not resolve to a function entry; skipping this hook. Injected enum defaults may be unavailable.");
        }
        ...
        /// <summary>
        /// Xref walks can yield a jmp into an epilogue, and detouring that aborts the CLR with 0x80131506.
        /// Only hook an address the module's own unwind data calls a function entry.
        ...
        private static nint AsFunctionEntry(nint ptr)
        {
            ...
            // Leaf functions carry no unwind record, so fall back to the alignment guess only when the
            // exception directory has nothing to say about the address.
            switch (MemoryUtils.ClassifyFunctionEntry(InjectorHelpers.Il2CppModule, ptr))
            {
                case MemoryUtils.FunctionEntry.Yes:
                    return ptr;
                case MemoryUtils.FunctionEntry.Interior:
                    return 0;
                default:
                    return (ptr & 0xF) == 0 ? ptr : 0;
            }
        }
